...
class ResolvedAtomCompleter(
    ...
    data class CallableReferenceResultTypeInfo(
    ...
    private fun complete(resolvedAtom: ResolvedAtom) {
        ...
        when (resolvedAtom) {
            ...
            is ResolvedCallableReferenceAtom -> completeCallableReferenceArgument(resolvedAtom)
    ...
    fun completeResolvedCall(resolvedCallAtom: ResolvedCallAtom, diagnostics: Collection<KotlinCallDiagnostic>): ResolvedCall<*>? {
        ...
        if (ErrorUtils.isError(resolvedCall.candidateDescriptor)) {
            if (lastCall is NewResolvedCallImpl<*>) {
    ...
    fun updateCallableReferenceResultType(
        callableCandidate: CallableReferenceCandidate,
        callableReferenceExpression: KtCallableReferenceExpression,
        rc: MutableResolvedCallAtom? = null
    ): CallableReferenceResultTypeInfo? {
        val freshSubstitutor = callableCandidate.freshSubstitutor ?: rc?.freshVariablesSubstitutor
        if (freshSubstitutor == null) return null
        ...
        val resultSubstitutor = if (callableCandidate.candidate.isSupportedForCallableReference()) {
            val firstSubstitution = typeParametersSubstitutor
            val secondSubstitution = resultSubstitutor
            ComposedSubstitutor(firstSubstitution, secondSubstitution)
    ...
    fun completeCallableReferenceCall(resolvedCall: NewCallableReferenceResolvedCall<*>): KotlinType? {
        val atom = resolvedCall.resolvedCallAtom
        if (atom is ResolvedCallableReferenceAtom) {
            return completeCallableReferenceArgument(atom)
        }
        atom as MutableResolvedCallAtom
        val callableReferenceExpression = atom.atom.psiKotlinCall.psiCall.callElement.parent as KtCallableReferenceExpression
        val candidate = atom.candidate ?: return null

        return completeCallableReference(callableReferenceExpression, candidate, resolvedCall.resultingDescriptor) { resolvedCall }
    }

    fun completeCallableReferenceArgument(resolvedAtom: ResolvedCallableReferenceAtom): KotlinType? {
        ...
        val candidate = resolvedAtom.candidate ?: return null
        val descriptor = when (candidate.candidate) {
        ...
        return completeCallableReference(callableReferenceExpression, candidate, descriptor, dataFlowInfo) {
            val reference = callableReferenceExpression.callableReference
            val call = CallMaker.makeCall(reference, it.explicitReceiver, null, reference, emptyList())
            NewCallableReferenceResolvedCall<CallableDescriptor>(resolvedAtom, call, typeApproximator, null)
    ...
    private fun completeCallableReference(
        ...
        getResolvedCall: (CallableReferenceResultTypeInfo) -> NewAbstractResolvedCall<*>,
    ): KotlinType? {
        ...
        val resultTypeInfo = if (recorderDescriptor != null) {
            ...
            updateCallableReferenceResultType(callableCandidate, callableReferenceExpression)
        }
        ...
        val resolvedCall = getResolvedCall(resultTypeInfo)
    ...
    private fun recordArgumentAdaptationForCallableReference(
        ...
        if (hasNonTrivialMapping || isCallableReferenceWithImplicitConversion(resolvedCall, callableReferenceAdaptation)) {
            ...
            } else if (resolvedCall is NewCallableReferenceResolvedCall<*>) {
                resolvedCall.setValueArguments(mappedArguments.toMap())
            }
        }
    }
