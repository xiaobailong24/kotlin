...
class NewCallableReferenceResolvedCall<D : CallableDescriptor>(
    val resolvedAtom: CallableReferenceResolvedAtom,
    ...
    override val resolvedCallAtom: MutableResolvedCallAtom? = when (resolvedAtom) {
        is CallableReferenceResolvedCallAtom -> resolvedAtom
        is CallableReferenceResolvedArgumentAtom -> resolvedAtom.candidate?.resolvedCall
    }

    override val psiKotlinCall: PSIKotlinCall = when (resolvedAtom) {
        is CallableReferenceResolvedCallAtom -> resolvedAtom.atom.psiKotlinCall
        is CallableReferenceResolvedArgumentAtom -> resolvedAtom.atom.kotlinCall.psiKotlinCall
    }
    ...
    override fun getCandidateDescriptor(): D = when (resolvedAtom) {
        is CallableReferenceResolvedCallAtom -> resolvedAtom.candidateDescriptor as D
        is CallableReferenceResolvedArgumentAtom -> resolvedAtom.candidate?.candidate as D
    }

    override fun getResultingDescriptor(): D = resultingDescriptor

    private var extensionReceiver = when (resolvedAtom) {
        is CallableReferenceResolvedCallAtom -> resolvedAtom.extensionReceiverArgument?.receiverValue
        is CallableReferenceResolvedArgumentAtom -> resolvedAtom.candidate?.extensionReceiver?.receiver?.receiverValue
    }

    private var dispatchReceiver = when (resolvedAtom) {
        is CallableReferenceResolvedCallAtom -> resolvedAtom.dispatchReceiverArgument?.receiverValue
        is CallableReferenceResolvedArgumentAtom -> resolvedAtom.candidate?.dispatchReceiver?.receiver?.receiverValue
    }
    ...
    override fun getExplicitReceiverKind(): ExplicitReceiverKind = when (resolvedAtom) {
        is CallableReferenceResolvedArgumentAtom -> resolvedAtom.candidate!!.explicitReceiverKind
        is CallableReferenceResolvedCallAtom -> resolvedAtom.explicitReceiverKind
    }
    ...
    override fun setResultingSubstitutor(substitutor: NewTypeSubstitutor?) {
        ...
        val sub = when (resolvedAtom) {
            is CallableReferenceResolvedCallAtom -> resolvedAtom.freshVariablesSubstitutor
            is CallableReferenceResolvedArgumentAtom -> resolvedAtom.candidate!!.freshSubstitutor!!
        }
    ...
    private fun substitutedResultingDescriptor(substitutor: NewTypeSubstitutor?): CallableDescriptor {
        val candidateDescriptor = when (resolvedAtom) {
            is CallableReferenceResolvedCallAtom -> resolvedAtom.candidateDescriptor
            is CallableReferenceResolvedArgumentAtom -> resolvedAtom.candidate!!.candidate
        }
    ...
    override val kotlinCall: KotlinCall
        get() = when (resolvedAtom) {
            is CallableReferenceResolvedArgumentAtom ->
                (resolvedAtom.candidate?.kotlinCall as CallableReferenceKotlinCallArgument).kotlinCall
            is CallableReferenceResolvedCallAtom -> resolvedAtom.atom
        }
