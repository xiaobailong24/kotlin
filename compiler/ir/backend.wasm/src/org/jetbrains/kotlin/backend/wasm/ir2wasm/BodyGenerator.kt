        ...
        if (function is IrSimpleFunction && function.isOverridable && !isSuperCall) {
            ...
            if (!klass.isInterface) {
                ...
                if (receiverClass == null || !receiverClass.isSubclassOf(klass)) {
                    body.buildRefCastStatic(context.referenceGcType(klass.symbol))
                }
                ...
                if (hierarchyDisjointUnions.hasUnionWithElement(symbol)) {
        ...
        when (function.symbol) {
            ...
            wasmSymbols.isInterface -> {
                val irInterface = call.getTypeArgument(0)!!.getClass()
                    ?: error("No interface given for wasmInterfaceId intrinsic")
                assert(irInterface.isInterface)
                if (hierarchyDisjointUnions.hasUnionWithElement(irInterface.symbol)) {
