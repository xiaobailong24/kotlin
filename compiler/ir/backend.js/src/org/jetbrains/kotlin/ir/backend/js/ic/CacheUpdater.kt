...
class CacheUpdater(
    ...
    private val dirtyFileStats = KotlinSourceFileMutableMap<EnumSet<DirtyFileStat>>()

    fun getDirtyFileStats(): KotlinSourceFileMap<EnumSet<DirtyFileStat>> = dirtyFileStats

    private fun MutableMap<KotlinSourceFile, EnumSet<DirtyFileStat>>.addDirtFileStat(srcFile: KotlinSourceFile, stat: DirtyFileStat) {
        when (val stats = this[srcFile]) {
            null -> this[srcFile] = EnumSet.of(stat)
            else -> stats.add(stat)
        }
    }
    ...
    private fun addFilesWithRemovedInlineImports(
        ...
        removedFiles.forEachFile { _, _, removedFileMetadata ->
            for ((dependentLib, dependentFiles) in removedFileMetadata.inverseDependencies) {
                val alreadyModifiedFiles = modifiedFiles[dependentLib] ?: emptyMap()
                val alreadyRemovedFiles = removedFiles[dependentLib] ?: emptyMap()
                val dependentCache = getLibIncrementalCache(dependentLib)
                ...
                for (dependentFile in dependentFiles.keys) {
                    when (dependentFile) {
                        ...
                        in extraModifiedFiles -> continue
                            ...
                            fileStats.addDirtFileStat(dependentFile, DirtyFileStat.REMOVED_IMPORTS)
                        }
                    }
                }
            }
    ...
    private fun loadModifiedFiles(): KotlinSourceFileMap<KotlinSourceFileMetadata> {
        ...
        val modifiedFiles = KotlinSourceFileMap(incrementalCaches.entries.associate { (lib, cache) ->
            ...
            newFiles.forEach { fileStats.addDirtFileStat(it, DirtyFileStat.ADDED_FILE) }
            removedFiles.forEach { fileStats.addDirtFileStat(it.key, DirtyFileStat.REMOVED_FILE) }
            dirtyFiles.forEach {
                if (it.key !in newFiles) {
                    fileStats.addDirtFileStat(it.key, DirtyFileStat.MODIFIED_IR)
                }
            }
        ...
        return addFilesWithRemovedInlineImports(modifiedFiles, KotlinSourceFileMap(removedFilesMetadata))
    }
    ...
    private fun rebuildDirtySourceMetadata(
        ...
        for ((lib, irModule) in loadedFragments) {
            ...
            for (fileDeserializer in moduleDeserializer.fileDeserializers()) {
                ...
                val importedSignatures = reachableSignatures.keys.toMutableSet()
                for (signature in allImplementedSignatures) {
                    if (signature in reachableSignatures) {
                        ...
                        importedSignatures.remove(signature)
                    }
                }
    ...
    private fun KotlinSourceFileMutableMap<UpdatedDependenciesMetadata>.addDependenciesWithRemovedInverseDependencies(
        ...
        for ((oldDependencyLibFile, oldDependencySrcFiles) in srcFileMetadata.oldDirectDependencies) {
            val dependencyCache = getLibIncrementalCache(oldDependencyLibFile)
    ...
    private fun KotlinSourceFileMutableMap<UpdatedDependenciesMetadata>.addDependentsWithUpdatedImports(
        ...
        for ((dependentLibFile, dependentSrcFiles) in srcFileMetadata.inverseDependencies) {
            val dependentCache = getLibIncrementalCache(dependentLibFile)
            ...
            for ((srcFile, srcFileMetadata) in srcFiles) {
                ...
                if (isSignatureUpdated || srcFileMetadata.importedInlineFunctionsModified) {
                    ...
                    if (isSignatureUpdated) {
                        fileStats.addDirtFileStat(srcFile, DirtyFileStat.UPDATED_EXPORTS)
                    }
                    if (srcFileMetadata.importedInlineFunctionsModified) {
                        fileStats.addDirtFileStat(srcFile, DirtyFileStat.UPDATED_INLINE_IMPORTS)
                    }
                } else {
