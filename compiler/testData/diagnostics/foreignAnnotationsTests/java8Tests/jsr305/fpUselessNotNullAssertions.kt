...
fun f(versionCode: Provider<Int?>, versionName: Provider<String?>) {
    ...
    val codeGetOrElseNonNull: Int = versionCode.getOrElse(1)<!UNNECESSARY_NOT_NULL_ASSERTION!>!!<!>
    // False positive: T is Int? so `null` is a valid parameter to getOrElse.
    val codeGetOrElseNull: Int? = versionCode.getOrElse(<!NULLABILITY_MISMATCH_BASED_ON_JAVA_ANNOTATIONS!>null<!>)
    ...
    // False positive: it is necessary, otherwise it doesn't compile with TYPE_MISMATCH.
    val nameGetOrElseNonNull: String = versionName.getOrElse("")<!UNNECESSARY_NOT_NULL_ASSERTION!>!!<!>
    // False positive: T is String? so `null` is a valid parameter to getOrElse.
    val nameGetOrElseNull: String? = versionName.getOrElse(<!NULLABILITY_MISMATCH_BASED_ON_JAVA_ANNOTATIONS!>null<!>)
}
