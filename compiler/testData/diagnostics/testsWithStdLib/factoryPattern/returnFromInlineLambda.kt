...
fun test_1(a: Array<String>, b: Boolean) {
    a.<!CANDIDATE_CHOSEN_USING_OVERLOAD_RESOLUTION_BY_LAMBDA_ANNOTATION!>myFlatMap { it.toList().ifEmpty { return } }<!>
    a.<!CANDIDATE_CHOSEN_USING_OVERLOAD_RESOLUTION_BY_LAMBDA_ANNOTATION!>myFlatMap {
        if (b) return
        it.toList()
    }<!>
}
...
fun test_2(a: Array<String>, b: Boolean) {
    a.<!CANDIDATE_CHOSEN_USING_OVERLOAD_RESOLUTION_BY_LAMBDA_ANNOTATION!>noInlineFlatMap { it.toList().ifEmpty { <!RETURN_NOT_ALLOWED!>return<!> } }<!>
    a.<!CANDIDATE_CHOSEN_USING_OVERLOAD_RESOLUTION_BY_LAMBDA_ANNOTATION!>noInlineFlatMap {
