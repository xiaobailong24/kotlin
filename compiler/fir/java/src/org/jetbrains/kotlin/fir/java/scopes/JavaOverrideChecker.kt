...
class JavaOverrideChecker internal constructor(
    ...
    private fun FirTypeRef?.isTypeParameterDependent(): Boolean =
        this is FirResolvedTypeRef && type.lowerBoundIfFlexible().originalIfDefinitelyNotNullable().isTypeParameterDependent()
    ...
    private fun FirTypeRef.extractTypeParametersTo(result: MutableCollection<FirTypeParameterRef>) {
        if (this is FirResolvedTypeRef) {
            type.lowerBoundIfFlexible().originalIfDefinitelyNotNullable().extractTypeParametersTo(result)
        }
    }
